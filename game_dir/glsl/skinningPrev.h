...
uniform vec4		u_PrevBonesArray[MAXSTUDIOBONES*3];
#else
...
#if defined( STUDIO_BONES_MATRICES )
mat4 Mat4PrevFromVec4Array( int boneIndex, float weight )
{
	mat4 m;

	m[0][0] = u_PrevBonesArray[boneIndex*3+0].x * weight;
	m[0][1] = u_PrevBonesArray[boneIndex*3+0].y * weight;
	m[0][2] = u_PrevBonesArray[boneIndex*3+0].z * weight;
	m[0][3] = 0.0;
	m[1][0] = u_PrevBonesArray[boneIndex*3+1].x * weight;
	m[1][1] = u_PrevBonesArray[boneIndex*3+1].y * weight;
	m[1][2] = u_PrevBonesArray[boneIndex*3+1].z * weight;
	m[1][3] = 0.0;
	m[2][0] = u_PrevBonesArray[boneIndex*3+2].x * weight;
	m[2][1] = u_PrevBonesArray[boneIndex*3+2].y * weight;
	m[2][2] = u_PrevBonesArray[boneIndex*3+2].z * weight;
	m[2][3] = 0.0;
	m[3][0] = u_PrevBonesArray[boneIndex*3+0].w * weight;
	m[3][1] = u_PrevBonesArray[boneIndex*3+1].w * weight;
	m[3][2] = u_PrevBonesArray[boneIndex*3+2].w * weight;
	m[3][3] = weight;

	return m;
}

mat4 ComputePrevSkinningMatrix( void )
{
	...
	mat4 boneMatrix = Mat4PrevFromVec4Array( boneIndex0, flWeight0 );
	if( boneIndex1 != -1 ) boneMatrix += Mat4PrevFromVec4Array( boneIndex1, flWeight1 );
	if( boneIndex2 != -1 ) boneMatrix += Mat4PrevFromVec4Array( boneIndex2, flWeight2 );
	if( boneIndex3 != -1 ) boneMatrix += Mat4PrevFromVec4Array( boneIndex3, flWeight3 );
	return boneMatrix;
#elif ( MAXSTUDIOBONES == 1 )
	return Mat4PrevFromVec4Array( 0, 1.0 );
#else
	return Mat4PrevFromVec4Array( int( attr_BoneIndexes[0] ), 1.0 );
#endif
}
#else
mat4 ComputePrevSkinningMatrix( void )

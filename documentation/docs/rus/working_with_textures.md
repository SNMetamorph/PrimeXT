...
- `Transparent`, нет внешней текстуры: будет использоваться альфа-тест
...
:::tip Примечание
Выбор подходящего режима прозрачности напрямую зависит от геометрии объекта. Альфа-блендинг работает корректно только на плоских (окна, платформы), либо выпуклых объектах (феры, цилинды, кубы и другие простые геометрические объекты). В остальных случаях, например, для листвы, травы, деревьев, и любой другой вогнутой или имеющей самопересечения геометрии, следует использовать режим alpha to coverage.

Либо же, вы можете просто на практике проверить, какой режим выглядит более корректно в вашем конкретном случае.
:::

...
size_t CPhysicNovodex::DecomposedShape::GetTrianglesCount() const
{
	return m_trianglesCount;
}

const std::vector<uint32_t>& CPhysicNovodex::DecomposedShape::GetIndexBuffer() const
{
	return m_indexBuffer;
}

const std::vector<physx::PxVec3>& CPhysicNovodex::DecomposedShape::GetVertexBuffer() const
{
...
void CPhysicNovodex::DecomposedShape::DebugDummyBox()
{
	PxBoxGeometry box;
	constexpr size_t vertsPerShape = 8;
	constexpr size_t trianglesCount = 12;
	...
	box.halfExtents = PxVec3(16.f, 16.f, 16.f);
	m_indexBuffer.reserve(m_indexBuffer.size() + trianglesCount * 3);
	m_vertexBuffer.reserve(m_vertexBuffer.size() + vertsPerShape);
	...
	for (size_t j = 0; j < trianglesCount; j++)
	{
		m_indexBuffer.push_back(trianglesIndices[j][0]);
		m_indexBuffer.push_back(trianglesIndices[j][1]);
		m_indexBuffer.push_back(trianglesIndices[j][2]);
		m_trianglesCount += 1;
	}
}

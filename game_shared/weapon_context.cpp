...
void CBaseWeaponContext::ItemPostFrame(uint32_t randomSeed)
{
	m_iRandomSeed = randomSeed;
	...
	else if ( !(m_pFuncs->CheckPlayerButtonFlag(IN_ATTACK|IN_ATTACK2) ) )
	{
		...
		if ( !IsUseable() && m_flNextPrimaryAttack < m_pFuncs->GetWeaponTimeBase() ) 
		{
			...
			if ( !(iFlags() & ITEM_FLAG_NOAUTOSWITCHEMPTY) && g_pGameRules->GetNextBestWeapon( m_pWeaponEntity->m_pPlayer, m_pWeaponEntity ))
			{
